# Intelligent Network Analyzer (Net Analyzer)
...
Made by <3 Arshan Mansuri for B.Tech Final Year Major Project.
...
## License / Attribution
Generated by Intelligent Network Analyzer - B.Tech Final Year Project by Arshan Mansuri. Customize licensing as needed for your deployment.
...
### Project Type
Defensive cybersecurity assessment platform with dual-mode scanning (external reconnaissance + internal LAN survey).

### Primary Goals
- Provide a single interface for external exposure analysis and internal asset visibility.
- Correlate active scanning with passive intelligence.
- Assign actionable risk scores and deliver professional reports.
- Maintain a history of external scans for trend analysis.

### Core Capabilities
1. External Recon (Public Targets)
   - Active scanning with Nmap.
   - Passive intelligence with Shodan.
   - CISA KEV exploitability flagging.
   - Risk scoring with severity and findings.
2. Internal Defense (Local Networks)
   - Auto-detect subnet and discover hosts.
   - ARP, ICMP, and TCP discovery with speed tuning.
   - Device fingerprinting and risk scoring.
   - Vendor enrichment via IEEE OUI (offline).
   - Device detail inspection (ports, ping, TLS info).
3. Reporting
   - External scan report (PDF).
   - Internal scan report (PDF).
   - Per-device report (PDF).
4. Scheduling and Alerts
   - Interval scheduling for external or internal scans.
   - Email alerts for high-risk scheduled results.
5. UI
   - Two workspaces: IP Recon and Network Scanner.
   - Terminal-style log for external results.
   - Inventory grid for internal assets.
   - Device detail modal with inspection tools.
...
### How to Use
#### IP Recon
1. Enter target domain or IP.
2. Choose scan type (quick/full) and speed (fast/standard/aggressive).
3. Click Run to start the scan.
4. Download PDF report after results appear.

#### Network Scanner
1. Choose speed.
2. Click Scan Local Network.
3. Open any device row for details.
4. Use Ping and Port tools in the modal.
5. Download internal or device report.

#### Scheduling
- External:
  - Provide target and interval minutes.
  - Click Schedule External.
- Internal:
  - Provide interval minutes.
  - Click Schedule Internal.
- Use View Schedules and Delete Schedule to manage jobs.

### Notes on Accuracy
- External scans depend on target accessibility and firewall rules.
- Internal scans depend on ARP visibility and device response to probes.
- Vendor data is offline IEEE OUI and may not match locally administered MACs.
